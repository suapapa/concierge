...
| `fe/` | Vite + React + TypeScript dashboard: `GET /api/v1/stat`, upload, delete, copy public URLs, and a header Settings toggle (next to Sign Out) that opens a dedicated settings view for API keys + admin “Users & quotas” (when `GET /admin/users` succeeds); `npm run dev` proxies `/api` to Concierge (see `fe/.env.example`) |
